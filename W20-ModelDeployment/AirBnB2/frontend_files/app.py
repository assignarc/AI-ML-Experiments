...
if st.button("Predict"):
    response = requests.post("https://AssignArc-RentalPricePredictionBackend.hf.space/v1/rental", json=input_data.to_dict(orient='records')[0])  # Send data to Flask API
...
if uploaded_file is not None:
    if st.button("Predict Batch"):
        response = requests.post("https://AssignArc-RentalPricePredictionBackend.hf.space/v1/rentalbatch", files={"file": uploaded_file})  # Send file to Flask API
        if response.status_code == 200:
